...
-- Order the result by pid in ascending order.
create or replace view tempQ6(pid, brand, name) as
select distinct staff.pid, insured_item.brand, person.firstname||' '||person.lastname
from Policy, 
insured_item,
Staff, 
Person
...
where not 
exists (select * from tempQ6 as staff2 where staff1.name = staff2.name and staff1.brand != staff2.brand) 
order by staff1.pid;
--=================================================================
-- Q7. List clients (their firstname, a space and then the lastname as one column called name) 
-- who hold policies to cover all brands of cars recorded in the database. 
-- [Ignore the policy status.] 
-- Order the result by pid in ascending order.

select p.pid, p.firstname||' '||p.lastname from Person as p, Client as C, Insured_by as ib
where c.cid = ib.cid and ib.pno = c.pno;

select * from Coverage as c1 where exists (select * from Coverage as c2 where
c1.pno = c2.pno and c1.cname != c2.cname);
...
-- Order the result by pno in ascending order. 
-- Exclude policies that contain no other policies in your output (i.e., exclude those with npolicies being 0). 
-- [Ignore the policy status.]
...
select distinct pno1, count(pno1) from (
	select client1.pno1, client1.pno3, count(*) matches from (
        select distinct client2.pno as pno1, client2.cname as cname, client3.pno as pno3, client3.cname as cname3 
        from Coverage client2
        inner join Coverage c3 on client2.cname = client3.cname 
        and client2.pno <> client3.pno
    )  
    client1
    group by client1.pno1 , client1.pno3) 
    sup
	inner join
(select pno, count(distinct cname) reqs from Coverage group by pno) 
sub
on sub.pno = sup.pno3 and sup.matches = sub.reqs
group by pno1;
